#!/bin/bash
# Bloggerator Cloud Run Deployment Script
# Run this from the project root directory (bloggerator/)
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "       authDomain: 'YOUR_PROJECT.firebaseapp.com',"
    echo "       projectId: 'YOUR_PROJECT',"
    echo "       storageBucket: 'YOUR_PROJECT.appspot.com',"
    echo "       messagingSenderId: 'YOUR_SENDER_ID',"
...
# DEPLOYMENT
# ============================================
echo "🚀 Bloggerator Cloud Run Deployment"
echo "===================================="
...
if [ ! -f "package.json" ]; then
    echo "❌ Error: Run this script from the bloggerator/ root directory"
    exit 1
fi
...
gcloud builds submit \
    ...
    --substitutions=_SERVICE_NAME=${SERVICE_NAME},_REGION=${REGION},_TAG=${TAG},_FIREBASE_API_KEY="${FIREBASE_API_KEY}",_FIREBASE_AUTH_DOMAIN="${FIREBASE_AUTH_DOMAIN}",_FIREBASE_PROJECT_ID="${FIREBASE_PROJECT_ID}",_FIREBASE_STORAGE_BUCKET="${FIREBASE_STORAGE_BUCKET}",_FIREBASE_MESSAGING_SENDER_ID="${FIREBASE_MESSAGING_SENDER_ID}",_FIREBASE_APP_ID="${FIREBASE_APP_ID}" \
    .
...
echo "   2. Update DNS records as shown in Cloud Console"
